# train_compositiongraph_final_fixed.py
import os
import pickle
import argparse
import numpy as np
...
def parse_args():
    ...
    parser.add_argument('--exp_name', type=str, default=None,
                        help='Experiment name (e.g., G_base, G_B_high). If not specified, will extract from data_dir')
...
def main():
    ...
    if args.exp_name:
        ...
        # 从data_dir自动提取（例如从 "data/graph_repair_experiment/G_base" 提取 "G_base"）
        exp_name = os.path.basename(os.path.normpath(args.data_dir))
        # 如果提取失败或为空，使用默认值
        if not exp_name or exp_name == '.':
            exp_name = 'unknown'
    
    # 确定mix类型（用于兼容性）
    mix_type = "alpine" if "alpine" in args.train_file else ("0mix" if "train_0" in args.train_file else "20mix")
    
    # ========== 生成包含图名称的输出目录（修改部分）==========
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = f'out/{exp_name}_d{args.n_embd}_seed{args.seed}_{timestamp}'
    ...
    header_msgs = [
        "="*60,
        f"Composition Training - Graph Repair Experiment",
        f"Experiment: {exp_name}",  # 新增：显示实验名称
