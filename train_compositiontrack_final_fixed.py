...
def main():
    ...
    if 'track_A' in args.data_dir:
        track = 'trackA'
    elif 'track_B' in args.data_dir:
        track = 'trackB'
    else:
        track = 'unknown'
    # 从train_file提取alpha值
    alpha_match = re.search(r'train_(\d+)\.bin', args.train_file)
    if alpha_match:
        alpha_val = int(alpha_match.group(1))
        alpha_str = f'alpha{alpha_val}'  # alpha0, alpha10, alpha50等
    else:
        alpha_str = 'unknown'

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = f'out/{track}_{alpha_str}_d{args.n_embd}_seed{args.seed}_{timestamp}'
