# train_composition_final.py (禁用Fused Optimizer的最终版)
import os
import pickle
import argparse
import numpy as np
...
# ... (从 parse_args 到 evaluate_composition 的所有代码保持不变) ...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="Train a GPT model for composition task")
    parser.add_argument('--data_dir', type=str, required=True, help='Directory containing the processed .bin files and meta.pkl')
    parser.add_argument('--train_file', type=str, required=True, help="Name of the training .bin file (e.g., train_0.bin or train_20.bin)")
        ...
        if len(parts) >= 2:
            try:
                ...
                if source in S1 and target in S2:
                    test_by_type['S1->S2'].append((source, target))
                elif source in S2 and target in S3:
                    test_by_type['S2->S3'].append((source, target))
                elif source in S1 and target in S3:
                    test_by_type['S1->S3'].append((source, target))
        ...
        for source, target in test_cases:
            ...
            if len(generated_path) >= 2 and generated_path[0] == source and generated_path[-1] == target:
                ...
                if path_is_connected:
                    if path_type == 'S1->S3':
                        if any(node in S2 for node in generated_path[1:-1]): is_valid = True
                    else:
                        is_valid = True
            if is_valid: correct_count += 1
        results[path_type] = {'correct': correct_count, 'total': len(test_cases), 'accuracy': correct_count / len(test_cases) if test_cases else 0}
    model.train()
    return results
...
def main():
    ...
    try:
        ...
        with open(os.path.join(data_dir, 'stage_info.pkl'), 'rb') as f: stages = pickle.load(f)['stages']
    ...
    # ##               ↓↓↓ 最终、决定性的修复 ↓↓↓                   ##
    # ####################################################################
    # 通过传递 device_type='cpu'，我们强制不使用可能有问题的 Fused AdamW 实现。
    # 实际的训练仍然会在GPU上，因为模型和数据都在GPU上。
